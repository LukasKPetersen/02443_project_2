...
class SingleServerIPP:
    ...
    def __init__(self, lambda_on, gamma1, gamma2, init_state='ON'):
        ...
        self.next_arrival_time = np.random.exponential(1/lambda_on) if init_state=='ON' else float('inf')
        self.set_sojourn_time()
    
    def set_sojourn_time(self):
        transition_rate = self.gamma1 if self.state=='ON' else self.gamma2
        self.curr_sojourn_time = self.next_event_time + np.random.exponential(1/transition_rate)
    ...
    def generate_arrivals(self):
        ...
        while self.next_event_time < self.curr_sojourn_time:
            ...
            self.next_event_time += np.random.exponential(1/self.lambda_on)
        return arrivals
